...
const FollowersPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchFollowers = async () => {
            setLoading(true);
            try {
                const endpoint = isOwnProfile ? '/followers' : `/followers/${username}`;
                ...
                if (!cancelled) console.error("Error fetching followers:", err);
            } finally {
        ...
        return (
            <div className="text-center py-16">
                ...
                <p className="text-text-muted text-sm max-w-xs mx-auto leading-relaxed">
                    {isOwnProfile
                        ? "Kamu belum memiliki pengikut. Mulai terhubung dengan teman-temanmu."
                        : "Pengguna ini belum memiliki pengikut."}
                </p>
            </div>
        );
    }
