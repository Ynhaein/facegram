...
const PostCard = ({ post }) => {
  ...
  return (
    <div className="bg-bg-secondary rounded-3xl overflow-hidden mb-6 max-w-xl mx-auto">

      {/* Header */}
      <div className="flex items-center justify-between px-5 py-4">
        <Link to={`/profile/${post.user.username}`} className="flex items-center gap-3">
          <div className="w-10 h-10 bg-primary rounded-xl flex items-center justify-center text-white font-semibold text-base shrink-0">
            {post.user.username?.charAt(0).toUpperCase()}
          </div>
          <p className="font-semibold text-text-primary text-sm">{post.user.username}</p>
        </Link>

        <button className="text-text-muted hover:text-text-primary transition-colors duration-200 cursor-pointer h-8 w-8 flex items-center justify-center rounded-lg hover:bg-bg-tertiary">
          <BsThreeDots size={18} />
        </button>
      </div>
      ...
      <div className="relative bg-black select-none">
        ...
        {hasMultipleImages && (
          <>
            <div className="absolute top-4 right-4 bg-black/60 backdrop-blur-sm text-white text-xs px-3 py-1.5 rounded-lg font-medium">
      ...
      <div className="px-5 pt-4 pb-2 flex items-center gap-6">
        <button
          ...
          className="text-xl transition-all duration-200 hover:scale-110 active:scale-90 cursor-pointer"
        >
          {liked ? <FaHeart className="text-red-500 drop-shadow-sm" /> : <FaRegHeart className="text-text-primary" />}
        </button>

        <button className="text-xl text-text-primary hover:text-text-secondary transition-colors duration-200 cursor-pointer">
          <FaRegComment />
        </button>
      </div>
